...
export function SleepwalkerView() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      content: 'I am the Sleepwalker Protocol Skill. How can I assist with state recovery or protocol analysis?' 
    }
  ]);
  ...
  async function handleSend(text: string) {
    ...
    try {
      ...
      const data = await sendAgentMessage('sleepwalker', text, newHistory);
  ...
  return (
    ...
    >
      ...
      <details style={{ marginTop: '1rem' }}>
        ...
        <pre style={{ whiteSpace: 'pre-wrap', background: '#05070d', padding: '1rem', borderRadius: 12, color: '#f8fafc', fontSize: '0.85rem', marginTop: '0.5rem' }}>{SLEEPWALKER_AGENT_PROMPT}</pre>
      </details>
    </PlaygroundShell>
  );
}
