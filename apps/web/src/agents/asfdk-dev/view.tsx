...
export function AsfdkDevView() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      content: 'I am your ASFDK-Dev pathway guide. Ready to help you build governed agents.' 
    }
  ]);
  ...
  async function handleSend(text: string) {
    ...
    try {
      const response = await fetch('/api/agents/asfdk-dev', {
        method: 'POST',
  ...
  return (
    ...
    >
      <details style={{ marginTop: '1rem' }}>
        ...
        <pre style={{ whiteSpace: 'pre-wrap', background: '#05070d', padding: '1rem', borderRadius: 12, color: '#f8fafc', fontSize: '0.85rem', marginTop: '0.5rem' }}>{ASFDK_DEV_AGENT_PROMPT}</pre>
      </details>
    </PlaygroundShell>
  );
}
