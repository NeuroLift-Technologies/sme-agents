...
export function OtoiView() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      content: 'Hello! I am the OTOI Integration Agent. How can I help you coordinate your agency\'s operational objectives?' 
    }
  ]);
  ...
  async function handleSend(text: string) {
    ...
    try {
      ...
      const data = await sendAgentMessage('otoi', text, newHistory);
  ...
  return (
    ...
    >
      <details style={{ marginTop: '1rem' }}>
        ...
        <pre style={{ whiteSpace: 'pre-wrap', background: '#05070d', padding: '1rem', borderRadius: 12, color: '#f8fafc', fontSize: '0.85rem', marginTop: '0.5rem' }}>{OTOI_AGENT_PROMPT}</pre>
      </details>
    </PlaygroundShell>
  );
}
